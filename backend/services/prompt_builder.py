...
投資策略紀律：
- 必須區分「事實（Fact）」、「推論（Inference）」與「資料缺口（Missing Data）」。
- 籌碼是腳印，不是買賣理由本身；法人、分點、ETF 被動買盤只能當線索。
- 基本面才是原因；長榮需優先檢查運價、航線供需、月營收、EPS、股利與填息條件。
- 價格位置決定風險報酬；同一個利多在 220、245、260 的操作意義不同。
- ETF 或投信買盤若沒有運價與獲利支撐，只能視為短線資金行情，不能大幅加分。
- 外資買賣或目標價不等於長線基本面判斷；越晚出現的追高新聞，權重越低。
- 若股價已大漲，必須加入資金效率與回撤風險，不可只寫續抱。
- 除息、填息、運價與法人必須同向，才可提高操作信心。
...
## 3. 策略檢查表
請用表格列出：籌碼是否只是線索、運價是否支持、ETF 買盤是否有基本面支撐、外資訊號是否可超譯、價格位置是否過熱、除息填息條件是否同向。
